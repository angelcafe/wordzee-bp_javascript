...
function fPalabrasEncontradas() {
    for (let x = 3; x < 8; x++) {
        ...
        for (let y = 0; y < pe; y++) {
            ...
            var txt = document.createTextNode(palabrasEncontradas[x][y] + ' - ' + total);
            td.appendChild(txt);
            tr.appendChild(td);
            var destino = document.getElementById("idPalabrasEncontradas");
            document.body.insertBefore(tr, destino.nextSibling);
        }
    }
}
