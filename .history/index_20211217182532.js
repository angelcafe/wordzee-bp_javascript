...
function ajax(enviar = null, alerta = false) {
    ...
    var userRequest = new Request('https://www.angelcastro.es/bpj/back/buscapalabras.php', initObject);
    fetch(userRequest)
        .then(response => {
...
function fPalabrasEncontradas() {
    ...
    vPalabrasEncontradas.forEach(function (palabra) {
        if (Array.isArray(palabrasEncontradas[palabra.longitud]) &&
            Array.isArray(puntosEncontrados[palabra.longitud])) {
