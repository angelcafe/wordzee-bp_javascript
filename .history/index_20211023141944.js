        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4) {
                if (xhr.status >= 200 && xhr.status < 300) {
                    palabras.responde = xhr.response;
                    if (alerta) {
                        palabras.alerta();
                    } else if (palabras.ajaxEnviar.borrar || palabras.ajaxEnviar.guardar) {
                        document.getElementById('principal').submit();
                    } else if (palabras.ajaxEnviar.cargar) {
                        palabrasCargadas(palabras.responde);
                    }
                } else {
...
function palabrasCargadas(datos) {
    console.log(datos);
}
