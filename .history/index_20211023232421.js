...
function fPalabrasEncontradas() {
    let tst = vPalabras.filter(function (a.nombre, a.longitud) {
        debugger;
    });


    vPalabras.forEach(function (palabra) {
        let letrasPalabra = palabra.nombre.split('');
        for (let y = 0; y < letrasPalabra.length; y++) {
            if (letrasDisponibles.indexOf(letrasPalabra[y]) === -1 || (palabra.nombre.split(letrasPalabra[y]).length - 1 > letrasDisponibles.join('').split(letrasPalabra[y]).length - 1)) {
                debugger;
                vPalabras.splice(vPalabras.indexOf(palabra.nombre), 1);
            }
        }
    });
    ...
    for (let x = 3; x < 8; x++) {
        ...
        palabrasEncontradas[x] = palabras[x].filter(function (palabra) {
