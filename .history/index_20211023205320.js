...
function fPalabrasEncontradas() {
    ...
    var maxRow = palabrasEncontradas.map(function(row){ return Math.max.apply(Math, row.length); });
    var max = Math.max.apply(null, maxRow);
    //let max = Math.max.apply(Math, palabrasEncontradas.map(function (el) { return el.length }));
    for (let y = 0; y < max; y++) {
        let tr = document.createElement("tr");
        for (let x = 3; x < 8; x++) {
            let td = document.createElement("td");
            td.className = 'menu palenc noselect';
            let txt = document.createTextNode(palabrasEncontradas[x][y] + ' - ' + total);
            td.appendChild(txt);
            tr.appendChild(td);
        }
