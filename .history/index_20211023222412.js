...
function fPalabrasEncontradas() {
    ...
    for (let x = 3; x < 8; x++) {
        ...
        palabrasEncontradas.puntos = new Array();

        palabrasEncontradas.nombre = palabras[x].filter(function (palabra) {
        ...
        for (let y = 0; y < palabrasEncontradas.nombre.length; y++) {
            let letras = palabrasEncontradas.nombre[y].split('');
            let total = 0;
            ...
            palabrasEncontradas.puntos[y] = total;
        }
        palabrasEncontradas.sort(function (a, b) {
    ...
    for (let y = 0; y < max; y++) {
        ...
        for (let x = 3; x < 8; x++) {
            ...
            if (palabrasEncontradas[x][y]) {
                let txt = document.createTextNode(palabrasEncontradas[x]['nombre'][y] + ' - ' + palabrasEncontradas[x]['puntos'][y]);
                td.appendChild(txt);
            }
            tr.appendChild(td);
        }
